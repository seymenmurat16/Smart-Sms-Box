...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        // Add a marke*r in Sydney and move the camera
        /*Double lon = extras.getDouble("lon");
        Double lat = extras.getDouble("lat");
        LatLng sydney = new LatLng(lat, lon);*/
        for(int i=messages2.size()-1;i>-1;i--){
            latitude = messages2.get(i).getLat();
            longitude = messages2.get(i).getLon();
            Log.i("Harita", " mesaj:  " +messages2.get(i).getBody() + " " + longitude + " lat : " + latitude );
            if(longitude != 0 && latitude != 0 ){
                LatLng mark = new LatLng(latitude,longitude);
                mMap.addMarker(new MarkerOptions().position(mark).title(""));
            }
        }
